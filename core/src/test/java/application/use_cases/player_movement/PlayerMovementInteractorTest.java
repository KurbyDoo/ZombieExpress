...
class PlayerMovementInteractorTest {
    ...
    class TrainMovementTests {
        ...
        void setUpTrainMovement() {
            ...
            train.setTotalFuel(50); // Give train some fuel
            player.setCurrentRide(train);
        }

        @Test
        @DisplayName("Should move on train when forward is pressed and train has fuel")
        void shouldMoveOnTrainWhenForwardPressedWithFuel() {
            GamePosition initialPosition = player.getPosition();
        ...
        void shouldNotMoveOnTrainWhenNoFuel() {
            ...
            GamePosition newPosition = player.getPosition();
            assertTrue(initialPosition.epsilonEquals(newPosition, 0.001f),
                "Position should remain unchanged when train has no fuel");
        }
        ...
        @Test
        @DisplayName("Train movement speed should match train speed")
        void trainMovementSpeedShouldMatchTrainSpeed() {
            train.setSpeed(30);
            GamePosition initialPosition = player.getPosition();
            ...
            GamePosition newPosition = player.getPosition();
            float distance = newPosition.dst(initialPosition);

            // The train speed is 30, so after 1 second we should move 30 units
            assertEquals(30.0f, distance, 0.5f,
